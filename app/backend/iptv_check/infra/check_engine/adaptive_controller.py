"""
自适应并发控制器
基于检测故障率（超时/限流429/断连等）动态调整并发度，解决高并发导致的级联超时与被源站限流问题
"""
import logging
...
class AdaptiveConcurrencyController:
    ...
    2. 窗口满时计算故障率（超时/限流/断连），决策降级或恢复
    3. 故障率 > high_timeout_threshold：并发度减半（不低于 min_threads）
    4. 连续 recovery_streak_required 个窗口故障率 < low_timeout_threshold：恢复至 max_threads 的75%
    5. 首个窗口禁止降级，复检阶段豁免降级
    """
    max_threads: int
    min_threads: int = 5
    ...
    def report_result(self, is_failure: bool) -> Optional[int]:
        """报告单个检测结果（True=故障：超时/429/断连等），返回调整后的并发度或None(未调整)"""
        ...
        failure_rate = self._timeouts_in_window / self._checked_in_window
        self._total_windows += 1
        adjustment = self._evaluate(failure_rate)

        self._checked_in_window = 0
        self._timeouts_in_window = 0
        return adjustment

    def _evaluate(self, failure_rate: float) -> Optional[int]:
        old_threads = self._current_threads

        if self._is_recheck:
            return None

        if failure_rate > self.high_timeout_threshold and self._total_windows > 1:
            ...
            if new_threads != old_threads:
                logger.info(
                    "[自适应并发] 降级: %d→%d, 故障率=%.1f%%, 原因=超时/限流/断连超过%.0f%%",
                    old_threads, new_threads, failure_rate * 100, self.high_timeout_threshold * 100,
                )
                return new_threads
        elif failure_rate < self.low_timeout_threshold:
