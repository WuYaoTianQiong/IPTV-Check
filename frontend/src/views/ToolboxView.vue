...
import { startM3uServer, stopM3uServer, getRecommendations, getRecommendM3u, getM3uState, convertText } from '../api'
...
onMounted(async () => {
  try {
    const res = await getM3uState()
    if (res.data.running) {
      m3uRunning.value = true
      m3uUrl.value = res.data.url
    }
  } catch {}
})
  ...
  try {
    if (m3uRunning.value) {
      ...
      if (data.url) {
        ...
        toast.error('M3U 服务启动失败')
      }
    }
  } catch (e) {
    toast.error('操作失败', e.response?.data?.message || e.message)
  }
}
...
async function loadRecommendations() {
  ...
  try {
    ...
    if (data.error) {
      toast.error('推荐失败', data.error)
      return
    }
    recTotal.value = data.total_channels
    recVariants.value = data.total_variants
  } catch (e) {
    toast.error('推荐失败', e.response?.data?.message || e.message)
  } finally {
