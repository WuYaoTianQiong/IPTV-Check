    ...
    <template v-else>
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div class="text-3xl font-bold" :class="report.valid_rate >= 70 ? 'text-green-500' : report.valid_rate >= 40 ? 'text-yellow-500' : 'text-red-500'">
              {{ report.valid_rate }}%
            </div>
          </CardContent>
        </Card>
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="(s, i) in report.source_ranking" :key="s.name" class="border-b last:border-0 hover:bg-accent/50">
                  ...
                  <td class="py-2 px-3 text-right text-green-600 dark:text-green-400">{{ s.valid }}</td>
                  <td class="py-2 px-3 text-right text-red-600 dark:text-red-400">{{ s.invalid }}</td>
                  <td class="py-2 px-3 text-right font-bold" :class="s.rate >= 70 ? 'text-green-500' : s.rate >= 40 ? 'text-yellow-500' : 'text-red-500'">{{ s.rate }}%</td>
