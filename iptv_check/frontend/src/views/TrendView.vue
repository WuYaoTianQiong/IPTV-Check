  ...
  <div class="space-y-6">
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>输入频道 ID 查看历史检测趋势</CardDescription>
      </CardHeader>
      <CardContent>
        <div class="flex gap-2 mb-4">
          <input
            ...
            type="number"
            placeholder="输入频道 ID"
            class="flex-1 rounded border bg-card px-3 py-2 text-sm"
            @keyup.enter="loadChannelTrend"
          />
          <button @click="loadChannelTrend" class="rounded bg-primary px-4 py-2 text-sm text-primary-foreground hover:bg-primary/90">
            查询
          </button>
        </div>
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="(item, i) in channelTrend.slice().reverse().slice(0, 50)" :key="i" class="border-t">
                  ...
                  <td class="px-3 py-2">
                    <span class="rounded px-1.5 py-0.5 text-xs" :class="item.is_valid ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'">
                      {{ item.is_valid ? '在线' : '离线' }}
                    </span>
                  </td>
...
async function loadChannelTrend() {
  ...
  try {
    const res = await getChannelTrend(channelId.value, daysRange.value)
